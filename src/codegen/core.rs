...
pub struct Codegen {
    ...
    pub is_main: bool,
    pub library: bool,
    pub host_functions: Vec<(String, crate::ast::Handler)>,
...
"#);
        ...
        if self.library {
            code = self.library_adjust(code);
            if is_main { code.push_str(&self.library_runtime()); }
        }
        code
    }
    ...
    pub fn compile_type(&self, ty: &Type) -> String {
        match ty {
            ...
            Type::Named(name) => rust_ident(name),
