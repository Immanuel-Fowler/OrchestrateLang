...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
            ...
            tokio::spawn(async move {
                ...
                loop {
                    let __inner_handle = tokio::spawn(async move {
                        loop {
                            let mut name = String::from("Orchestrate");
                        let mut greeting = OrchAdd::orch_add(OrchAdd::orch_add(String::from("Hello, "), name), String::from("!"));
                        print_val(greeting);
                        let mut x = 10;
                        let mut y = 20;
                        let mut sum = OrchAdd::orch_add(x, y);
                        print_val(OrchAdd::orch_add(String::from("10 + 20 = "), to_string(sum)));
                        stop_orch();
                        }
                    });
    ...
    orchestrator_main(vec![]).await;
    let payload_eval = std::sync::Arc::new(vec![p]);
    if let Ok(handlers) = get_registry_update_orchestrator().lock() {
        for tx in handlers.iter() {
            if tx.try_send(std::sync::Arc::clone(&payload_eval)).is_err() {
                eprintln!("[orchestrate] warning: dropped event 'update_orchestrator' — subscriber channel full");
            }
        }
    }
    loop {
