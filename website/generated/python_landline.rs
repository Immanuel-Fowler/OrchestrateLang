...
#[derive(Debug)]
pub enum CounterMsg {
    Add { amount: i64, reply_to: tokio::sync::oneshot::Sender<i64> },
}

#[derive(Default, Debug)]
pub struct CounterLatest {
    add: std::sync::Mutex<Option<i64>>,
}

#[derive(Clone, Debug)]
pub struct CounterClient {
    tx: tokio::sync::mpsc::Sender<CounterMsg>,
    latest: std::sync::Arc<CounterLatest>,
}

impl CounterClient {
    pub async fn add(&self, amount: i64) -> i64 {
        let (reply_tx, reply_rx) = tokio::sync::oneshot::channel();
        let _ = self.tx.send(CounterMsg::Add { amount, reply_to: reply_tx }).await;
...
pub fn start_Counter() -> CounterClient {
    ...
    let __latest = std::sync::Arc::new(CounterLatest::default());
    let latest = __latest.clone();
    tokio::spawn(async move {
        ...
        'restart: loop {
            ...
            while let Some(msg) = rx.recv().await {
                match msg {
                    CounterMsg::Add { amount, reply_to } => {
...
async fn orchestrator_main() {
    // orch:7:5
        ...
        // orch:8:5
        print_val(to_string(counter.add(3).await));
        // orch:9:5
        print_val(to_string(counter.add(4).await));
        // orch:10:5
        stop_orch();
}
