...
async fn orchestrator_main(procs: Vec<ProcessRef>) {
    struct ActiveState {
        ...
        // orch:29:5
        tokio::time::sleep(std::time::Duration::from_millis(3000 as u64)).await;
        // orch:30:5
        stop_orch();
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
            ...
            tokio::spawn(async move {
                ...
                loop {
                    let __inner_handle = tokio::spawn(async move {
                        loop {
                            print_val(String::from("flaky worker: starting iteration"));
                        tokio::time::sleep(std::time::Duration::from_millis(500 as u64)).await;
                        print_val(String::from("flaky worker: done with iteration"));
                        }
                    });
                    match __inner_handle.await {
                        ...
                        {
        // orch:9:5
            print_val(OrchAdd::orch_add(String::from("flaky worker crashed: "), e))
    };
        ...
        std::sync::Arc::new(move || {
            tokio::spawn(async move {
                ...
                loop {
                    let __inner_handle = tokio::spawn(async move {
                        loop {
                            print_val(String::from("monitor: heartbeat"));
                        tokio::time::sleep(std::time::Duration::from_millis(2000 as u64)).await;
                        }
                    });
                    match __inner_handle.await {
                        Ok(_) => break,
                        ...
                        {
        // orch:17:5
            print_val(OrchAdd::orch_add(String::from("monitor crashed and will restart: "), e))
    };
                        let __delay_ms = (1000u64 * 2u64.saturating_pow(__restart_count.min(5))).min(30000);
                        tokio::time::sleep(std::time::Duration::from_millis(__delay_ms)).await;
                    }
        ...
        std::sync::Arc::new(move || {
            tokio::spawn(async move {
                ...
                loop {
                    let __inner_handle = tokio::spawn(async move {
                        loop {
                            print_val(String::from("no-restart worker: tick"));
    ...
    orchestrator_main(vec![flaky_worker, persistent_monitor, no_restart_worker]).await;
    loop {
