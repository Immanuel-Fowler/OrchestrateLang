...
static REGISTRY_HANDLE_ALERT: std::sync::OnceLock<std::sync::Mutex<Vec<tokio::sync::mpsc::Sender<std::sync::Arc<String>>>>> = std::sync::OnceLock::new();
fn get_registry_handle_alert() -> &'static std::sync::Mutex<Vec<tokio::sync::mpsc::Sender<std::sync::Arc<String>>>> {
    REGISTRY_HANDLE_ALERT.get_or_init(|| std::sync::Mutex::new(Vec::new()))
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        std::sync::Arc::new(move || {
            tokio::spawn(async move {
                ...
                loop {
                    ...
                    let mut service = counter::start_CounterService();
                    let __inner_handle = tokio::spawn(async move {
                        loop {
                            print_val(String::from("Starting Counter Service..."));
                        let mut val1 = service.increment(5).await;
                        print_val(OrchAdd::orch_add(String::from("Result 1: "), to_string(val1)));
                        let mut val2 = service.increment(10).await;
                        print_val(OrchAdd::orch_add(String::from("Result 2: "), to_string(val2)));
                        let mut final_val = service.get_count().await;
                        print_val(OrchAdd::orch_add(String::from("Final Count: "), to_string(final_val)));
                        let payload_eval = std::sync::Arc::new(String::from("Overheat detected!"));
    if let Ok(handlers) = get_registry_handle_alert().lock() {
        for tx in handlers.iter() {
            if tx.try_send(std::sync::Arc::clone(&payload_eval)).is_err() {
                eprintln!("[orchestrate] warning: dropped event 'handle_alert' — subscriber channel full");
            }
        }
    }
    ...
    let mut p2 = {
        std::sync::Arc::new(move || {
            let (tx, mut rx) = tokio::sync::mpsc::channel::<std::sync::Arc<String>>(100);
            get_registry_handle_alert().lock().unwrap().push(tx);
            tokio::spawn(async move {
                while let Some(msg) = rx.recv().await {
                    let msg = (*msg).clone();
                    tokio::spawn(async move {
                        {
        // orch:32:5
            print_val(OrchAdd::orch_add(String::from("ALERT PROCESS BLOCK: "), msg))
    }
                    });
                }
            });
    ...
    p2();
    orchestrator_main(vec![p1]).await;
    loop {
