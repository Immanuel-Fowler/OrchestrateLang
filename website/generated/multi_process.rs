...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
            ...
            tokio::spawn(async move {
                ...
                loop {
                    let __inner_handle = tokio::spawn(async move {
                        loop {
                            print_val(String::from("Process 1 running: Tick A"));
                        tokio::time::sleep(std::time::Duration::from_millis(100 as u64)).await;
                        print_val(String::from("Process 1 running: Tick B"));
                        tokio::time::sleep(std::time::Duration::from_millis(100 as u64)).await;
                        }
                    });
        ...
        std::sync::Arc::new(move || {
            tokio::spawn(async move {
                ...
                loop {
                    let __inner_handle = tokio::spawn(async move {
                        loop {
                            print_val(String::from("Process 2 running: Step 1"));
                        tokio::time::sleep(std::time::Duration::from_millis(150 as u64)).await;
                        print_val(String::from("Process 2 running: Step 2"));
                        tokio::time::sleep(std::time::Duration::from_millis(150 as u64)).await;
                        stop_orch();
                        }
                    });
    ...
    orchestrator_main(vec![p1, p2]).await;
    loop {
