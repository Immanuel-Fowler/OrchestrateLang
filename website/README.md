...
| `compare.html` | When to use what: OrchestrateLang beside hand-written Tokio, Erlang/OTP, Akka, and Temporal, plus a gallery of five orchestration jobs written in each. Gallery snippets live in `<script type="text/plain" data-ex data-lang>` blocks near the end of the file, so code needs no escaping; the cards and the language switcher are generated from them |
